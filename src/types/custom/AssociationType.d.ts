...
export type AssociationToObject<T> = T extends () =>
  | HasMany<any, infer R>
  | BelongsToMany<any, infer R>
  ? R extends Model
    ? ModelToObject<R>[]
    : never[]
...
export type AssociationToModel<T> = T extends () =>
  | HasMany<any, infer R>
  | BelongsToMany<any, infer R>
  ? R extends Model
    ? RealModel<R>[]
...
export type IncludeFn<
  ...
  ScopeNames = keyof TModelScopes<M>,
> = (
  options?: Omit<IncludeOptions<M['_attributes']>, 'association' | 'model' | 'as' | 'through'> & {
...
type Singular<K> = K extends `${string}ss` ? K : K extends `${infer R}s` ? R : K;
...
export type AssociationMethods<M extends Model, T extends object = M['_type_assocs']> = {
  [K in keyof T as T[K] extends () => HasMany | BelongsToMany
    ...
    : `get${Capitalize<string & K>}`]: T[K] extends () => HasOne<any, infer R>
...
} &
  {
    [K in keyof T as T[K] extends () => HasMany | BelongsToMany
      ...
      : `set${Capitalize<string & K>}`]: T[K] extends () => HasOne<any, infer R>
  ...
  } &
  {
    [K in keyof T as T[K] extends () => HasMany | BelongsToMany
      ...
      : `create${Capitalize<string & K>}`]: T[K] extends () => HasOne<any, infer R>
  ...
  } &
  {
    [K in keyof T as T[K] extends () => HasMany | BelongsToMany
      ...
      : never]: T[K] extends () => HasMany<any, infer R>
  ...
  } &
  {
    [K in keyof T as T[K] extends () => HasMany | BelongsToMany
      ...
      : never]: T[K] extends () => HasMany<any, any>
